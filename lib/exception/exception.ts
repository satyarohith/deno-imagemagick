...
import { ExceptionSeverity } from "./exception-severity.ts";
...
export class Exception {
  ...
  check<TReturnType>(
    ...
    if (
      Exception.isRaised(this.pointer) &&
      Exception.isErrorSeverity(this.pointer)
    ) {
      return error();
    }
  ...
  private static isErrorSeverity(exception: Pointer): boolean {
    const severity = ImageMagick._api._MagickExceptionHelper_Severity(
      exception.value,
